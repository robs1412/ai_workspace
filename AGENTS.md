...
- AI Manager and task-mode prompt inputs must be recorded in DB-backed `koval_crm.ai_manager_inputs` through `scripts/ai_manager_chat_entry_adapter.php`. Do not append to `daily-inputs/*.md`; the Markdown daily-input trail is stopped and must not be used as a projection surface.
...
- Task-mode inputs that change direction, approve a blocker, add a durable workflow rule, or start substantive work must be mirrored through `scripts/ai_manager_chat_entry_adapter.php` with `--source-channel task-mode-chat` and the related Task Flow key when one exists; the adapter records to DB only.
