...
- Summary cadence clarification approved by Robert on 2026-04-16: default scheduled summary emails for both Frank and Avignon are morning summaries only. Evening roundups/end-of-day summaries are off by default for both workers unless Robert explicitly re-approves a specific one-off or recurring evening cadence. Do not treat task completion confirmations as evening roundups.
...
- When a task is auto-handled under this model, log the action in the appropriate workspace TODO/log and include it in the next morning summary or task-specific completion note so the human owner can audit what happened.
