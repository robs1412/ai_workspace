...
- Stale-state reconciliation rule clarified by Robert on 2026-04-23: if a worker already completed the business action, sent the owner report, filed the source item, recorded the blocker, or otherwise closed the loop in durable state, Task Manager and Decision Driver must reconcile the board/session state internally. Do not continue to present that session as a blocker, decision, or open work item just because the board status was left at `waiting-on-external`, `needs-input`, or another stale label. First recover the underlying outcome from session history, TODO/HANDOFF, project-hub notes, drafts/logs, or git state; then close, park as review-ready, or relabel the session appropriately.
