...
label="com.koval.hr-eligibility-weekly-report"
root="/Users/werkstatt/ai_workspace"
script="$root/scripts/hr_eligibility_weekly_report.py"
log_dir="$root/tmp/hr-eligibility-weekly-report"
...
Installs a due-date-aware Codex HR benefits eligibility reporter.
Default: no-GUI system LaunchDaemon running as admin every 21600 seconds.
The script only emails Sebastian when OPS task 371791 is due, then advances the recurring task.
USAGE
}
...
if [[ "$interval" -lt 3600 ]]; then
  echo "interval_seconds must be >= 3600 for this due-date-aware reporter" >&2
  exit 64
fi
