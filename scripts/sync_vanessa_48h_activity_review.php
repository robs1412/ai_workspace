...
function build_body(string $targetDateLabel, array $events): string
{
    $lines = [
        'Hi Vanessa,',
        '',
        'This is your 48-hour Outreach activity review reminder for staffed tastings from ' . $targetDateLabel . '.',
        '',
        'Please confirm each tasting below produced the right Portal activity on the right account. If anything is missing, remind the staff member or enter/follow through as required.',
        '',
        'Events to review:',
        '',
    ];
    ...
    $lines = array_merge($lines, [
        '',
        'Required result:',
        '- verify the activity exists in Portal for each event/account',
        '- if an activity is missing, send the follow-up reminder or record one exact blocker',
...
$eventStmt->execute([$targetDate->format('Y-m-d')]);
...
$sourceRef = 'vanessa-post-tasting-activity-review-' . $date->format('Y-m-d') . '-2300';
$queued = 0;
$skipped = 0;
$rowsForDbSync = [];
...
if (empty($staffedEvents)) {
    ...
    $targetDateLabel = $targetDate->format('l, F j');
    $eventIdList = array_map(static fn(array $row): int => (int) ($row['id'] ?? 0), $staffedEvents);
    $body = build_body($targetDateLabel, $staffedEvents);
    $newRow = [
        'id' => $sourceRef,
        'kind' => 'outreach_post_tasting_activity_review',
        'status' => 'pending',
        'ops_task_id' => 368771,
        'due_at' => $date->format('Y-m-d') . 'T23:00:00-05:00',
        'owner' => 'Robert Birnecker',
        'worker' => 'Vanessa Sterling',
        'dependency' => '48-hour Outreach activity review for staffed tastings on ' . $targetDate->format('Y-m-d'),
        'resolution_checks' => [],
        ...
        'email' => [
            ...
            'subject' => '48-hour activity review for outreach tastings from ' . $targetDateLabel,
            'body' => $body,
        ],
        ...
        'source_links' => 'OPS task 368771 + Outreach event IDs ' . implode(', ', $eventIdList),
        'approval_gates' => 'Internal scheduled reminder only; no external send beyond Vanessa/Robert.',
        'verification_readback' => 'Generated from live staffed Outreach event data for 48-hour activity review.',
        'papers_projection' => 'not_applicable',
        'next_update' => 'Pending 11:00 PM scheduled-action queue for the approved send cycle.',
