# Social Posting Task Lane

Updated: 2026-05-19

## Purpose

Create a durable social-posting lane for communications work so the team has one repeatable owner, one repeatable source packet, and one repeatable send path.

## Decision

- `Claude` owns the social-posting lane.
- `Communications Manager` supplies copy/tone/editing support.
- `Email Coordinator` stays out of the lane unless the social asset is also being repurposed into a sendable artifact.
...
- Cadence: weekly
- Deliverable: a concise social posting packet or send-ready post draft
- Proof: owner-visible Message-ID plus source readback, or one exact blocker

## Source Packet

Use these sources when drafting the social post packet:
...
- Keep the lane focused on short campaign-style or highlight-adjacent social posts.
