...
- Root-cause candidate isolated after rollback: the new frontend image did not preserve prior hashed static assets. Old cached Portal HTML requested `/js/app.e98ec78f.js` and `/js/chunk-vendors.195a95d8.js`; `v20260415b` returned 404 for both. Portal commit `1595a1af` changes frontend image builds to preserve assets from `PREVIOUS_FRONTEND_IMAGE` before overlaying the new dist.
- Handoff for phone/Task Manager continuation is recorded in `HANDOFF.md` under `2026-04-15 Portal reset-gate hotfix handoff`. Hotfix build command in progress: `PREVIOUS_FRONTEND_IMAGE=koval-crm-frontend:v20260415 ./scripts/build.sh all v20260415c`. Verify sidecar old asset URLs return 200 before deploying `v20260415c`; immediate rollback remains `./deploy/scripts/rollback.sh all v20260415`.
- 2026-04-17 status-only closeout review: local docs/code/log artifacts do not prove that Portal-user password resets are complete. The project remains open because closure requires an approved live read-only rollout status check or the scheduled OPS follow-up `366809` on 2026-04-29. No email, auth/session/reset flag, deploy, live pull, secret, or production DB action was performed during this review.
