...
function columbus_week_day_id(string $date): int
{
...
function columbus_google_token_user_id(PDO $pdo): ?int
{
...
function columbus_upsert_google_link(PDO $pdo, int $eventId, string $uid): void
{
...
$eventName = 'Columbus Ave. Wine & Spirits tasting';
$eventDate = '2026-07-10';
$eventStart = '16:00';
$eventEnd = '19:00';
$location = 'Columbus Ave. Wine & Spirits, 730 Columbus Avenue, New York, NY 10025';
$accountId = 39226; // Columbus Ave Spirits
$createdBy = 1332; // Codex
$eventHost = 1343; // Vanessa Sterling
$assignedUserId = 1286; // Benjamin Goodman
$cotGroupId = 169; // COTeam
$sourceMessageId = '<df9db7e8667fc7f4217c19b81ca2c652@koval-distillery.com>';
$notes = implode("\n", [
    'Source: Benjamin Goodman email to Vanessa Sterling, subject "Tasting Confirmed at Columbus Ave. Wine & Spirits 7/10", Message-ID ' . $sourceMessageId . '.',
    'Benjamin confirmed the tasting at Columbus Ave. Wine & Spirits on Friday, July 10, 2026, from 4:00 PM to 7:00 PM.',
    'Store address verified from the Columbus Ave. Wine & Spirits public contact page on 2026-05-29.',
    'Assigned coverage: Benjamin Goodman.',
]);
$importantInformation = 'Confirmed tasting. Benjamin Goodman is assigned for 4:00 PM-7:00 PM.';
...
try {
    ...
    $dupeStmt->execute([$eventDate, $accountId, '%Columbus Ave%', '%Columbus Ave%', '%' . $sourceMessageId . '%']);
    ...
    if ($eventId > 0) {
        ...
        $updateEvent->execute([
            ...
            $eventEnd,
            'Benjamin Goodman',
            'benjamin.goodman@koval-distillery.com',
            '(646) 342-4733',
            null,
            $notes,
    ...
    $shiftNotes = 'Outreach: Columbus Ave. Wine & Spirits tasting - Benjamin Goodman coverage';
    if ($shiftId > 0) {
        ...
        $updateShift->execute([
            columbus_week_day_id($eventDate),
            $eventDate,
            $eventDate,
            $eventStart,
            $eventEnd,
...
try {
    $tokenUserId = columbus_google_token_user_id($eventPdo);
    if ($tokenUserId === null) {
    ...
    columbus_upsert_google_link($eventPdo, $eventId, $uid);
...
echo json_encode(['event' => $row, 'google_sync' => $googleSync], JSON_PRETTY_PRINT) . "\n";
