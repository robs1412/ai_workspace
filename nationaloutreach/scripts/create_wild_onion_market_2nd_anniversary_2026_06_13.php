...
function wild_onion_week_day_id(string $date): int
{
...
function wild_onion_upsert_google_link(PDO $pdo, int $eventId, string $uid): void
{
...
$eventName = 'Wild Onion Market 2nd Anniversary Event';
...
$sourceMessageId = 'caatx44bajyq=heh-ktjrp-wvj4peu7audnfhj8j2ycfae7cpig@mail.gmail.com';
$sourceRef = 'taskflow-b5c9a2f7f67624d7';
$location = 'Wild Onion Market, 7007 N. Clark Street, Chicago, IL 60626';
$notes = implode("\n", [
    'Source: Robert Birnecker email to Dereck Atwater and Vanessa Sterling, subject "Re: Wild Onion Market 2nd Anniversary Event", Message-ID ' . $sourceMessageId . '.',
    'Robert asked Vanessa to add the event to the outreach calendar and send an event notice to the team as an open shift.',
    'Event request from Dereck/Taylor/Ryan: Wild Onion Market second anniversary celebration on Saturday, June 13, 2026, 11:00 AM-5:00 PM.',
    'Dereck is unavailable; Robert noted KOVAL may not have someone there for all 6 hours and wants coverage checked over the next few days.',
    'If staffing cannot cover the table, Dereck offered to drop off swag the day prior for Wild Onion Market to hand out.',
    'External contact from forwarded request: Ryan Whitney, Store Manager, storemanager@wildonionmarket.com.',
    'RNDC contact from forwarded request: Taylor Oller, Taylor.Oller@RNDC-USA.COM.',
    'Task Flow: ' . $sourceRef . '.',
]);
$importantInformation = 'Open COTeam shift for Wild Onion Market anniversary tabling; coverage may be partial. If no team member can cover, Dereck offered swag drop-off the day before.';
...
try {
    ...
    $dupeStmt->execute([$eventDate, $accountId, '%Wild Onion%Anniversary%', '%Wild Onion%', '%' . $sourceMessageId . '%']);
    ...
    if ($eventId > 0) {
        ...
        $updateEvent->execute([
            ...
            $eventEnd,
            'Ryan Whitney',
            'storemanager@wildonionmarket.com',
            '',
            null,
            $notes,
    ...
    $shiftNotes = 'Outreach: Wild Onion Market 2nd Anniversary Event - open COTeam coverage';
    if ($shiftId > 0) {
        ...
        $updateShift->execute([
            wild_onion_week_day_id($eventDate),
            $eventDate,
            $eventDate,
            $eventStart,
            $eventEnd,
...
try {
    ...
    wild_onion_upsert_google_link($eventPdo, $eventId, $uid);
