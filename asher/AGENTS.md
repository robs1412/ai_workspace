# AGENTS.md - asher Workspace

Scope: Applies to everything under `asher/`.

## Purpose

This workspace belongs to the Asher email worker for `asher@thecultivater.com`.

## Management

- Avignon manages this worker by default.
- The Cultivater is managed by Sonat. Send Asher approval packets, draft reviews, blocker details, route-info notes, completion reports, and follow-up packets to `sonat@kovaldistillery.com` by default, not Robert. Robert may receive brief chat status when he asks, but do not email Asher operational packets to Robert unless he explicitly says to copy himself in addition to Sonat.
...
- Current structure is Avignon-managed, Sonat-owned, and finish-contract driven. Asher may process approved sends, create/report owner questions or blockers, record no-action/source-proof outcomes, and route substantive work to visible Workspaceboard/Task Flow workers.
...
Use `../docs/email-workers/2026-06-07-shared-vanessa-style-fast-path-reliability.md` for the compact finish contract. Asher's current route table is:
...
Finish every Asher packet as `sent`, `archived/no-action`, `routed`, `closed_with_proof`, or `blocked`; include the live proof surface or the exact runtime gap when a mailbox mutation was not possible.
...
- For editorial work, Asher owns heritage, organic farming, food and drink, slow travel, provenance, stewardship, and craft-lineage angles for The Cultivater.
