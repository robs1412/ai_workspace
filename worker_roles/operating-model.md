...
| Barrel Sales Manager | Human-supervised on-demand specialist worker | `ws ai` for coordination; `ws sales` for Salesreport WH Barrel Program verification; `ws portal` for POS/Samples sample-request entry; `ws avignon` for Sonat-facing intake/reporting | `Barrel Sales Manager`, `barrel sales`, `WH Barrel Program`, `barrel sample`, `reserve barrel`, `mark barrel sold`, `bottling schedule` | `worker_roles/barrel-sales-manager.md`, `avignon/BARREL_PROGRAM_GUIDANCE.md`, Salesreport/Portal TODOs, OPS/Portal task ids, Avignon handoff/logs |
