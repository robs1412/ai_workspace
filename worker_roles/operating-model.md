...
- Apply the Vanessa-style reliability mechanic from `docs/email-workers/2026-06-07-shared-vanessa-style-fast-path-reliability.md` to Frank, Avignon, Ezra, Asher, and Venetia: classify by a compact route table first, allow `routine-if-clear` only where already approved, and close every packet as `sent`, `archived/no-action`, `routed`, `closed_with_proof`, or `blocked` with one exact human-readable blocker. This is a shared mechanic, not new send, filing, body-read, runtime, credential, or production authority.
