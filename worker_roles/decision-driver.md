...
- Do not surface a question merely because a session is `waiting`. First determine whether the waiting state is stale, superseded, auto-routable, or already answered.
...
- Queue-quality rule: Robert should see only specific, answerable `Needed / Next / Decision` packets. If the packet is generic, recover the real question or keep it internal until it is concrete.
- Board-noise rule: when many waiting items appear, bias toward internal reconciliation and batching. Do not ask Robert to bless routine cleanup one item at a time.
