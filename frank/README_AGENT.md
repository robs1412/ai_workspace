...
Robert-facing drafting note: for quick answers, answer directly and do not send a separate routing receipt. For routed work, the receipt is delayed 10 minutes and suppressed when the closeout arrives first. Complete Frank emails should include a normal greeting, then start the body with the point without writing the literal label `Point first:`, and end with `Best,`, a blank line, then `Frank` before Frank's full signature block. Keep visible session/task ID plus session/task title in captured/routed responses when available, split multi-part replies into short paragraphs with a blank line between sections, and keep the signature lines separate when drafting manually: Phone line, Website line, then linked `X | Instagram | Facebook` labels without raw URLs printed next to the labels. If there is one concrete blocker question Robert must answer, give that question its own short paragraph so it is more prominent than the surrounding context. Link those labels to the KOVAL accounts in the shared signature directive: X -> `http://www.x.com/kovaldistillery`, Instagram -> `https://www.instagram.com/kovaldistillery`, and Facebook -> `https://www.facebook.com/kovaldistillery`.
...
- Frank signature guidance requires phone, website, then one social-label line. In HTML email, `X | Instagram | Facebook` should be linked labels pointing to X `http://www.x.com/kovaldistillery`, Instagram `https://www.instagram.com/kovaldistillery`, and Facebook `https://www.facebook.com/kovaldistillery`; the social URLs should not be printed next to the labels. Runtime/template enforcement remains a separate implementation slice unless already covered by an approved runtime path.
