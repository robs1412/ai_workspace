...
Last Updated: 2026-05-27 10:02:00 CDT (Machine: Macmini.lan)
...
- If the user says `check ToDo`, interpret that as DB/task intake first: check OPS/Portal task IDs and Workspaceboard Task Flow state. Use local Markdown only when Robert explicitly asks for it or when no DB-backed record exists yet.
...
## DB Task Intake (Required)

- At the start of substantive work, check the DB-backed task spine: OPS/Portal task IDs when present and Workspaceboard Task Flow state.
- Check OPS tasks assigned to `Codex` (user id `1332`) and process only tasks created by user id `1` (`smcreatorid = 1`) when Robert explicitly triggers OPS intake.
...
- Do not run background polling daemons. Intake is manual and user-triggered.
...
- Task mode is not no-record mode. Even when work is done directly in chat/terminal, record the work in the appropriate durable local surface for that workspace, and the DB-backed task spine is required for substantive task execution. `HANDOFF.md`, project-hub notes, or legacy local projections may add context, but they do not replace the Task Flow / OPS / Portal DB record.
- Task-mode inputs that change direction, approve a blocker, add a durable workflow rule, or start substantive work must be mirrored through `scripts/ai_manager_chat_entry_adapter.php` with `--source-channel task-mode-chat` and the related Task Flow key, so the input lands in DB-backed `ai_manager_inputs` in addition to the Task Flow packet. Do not write `daily-inputs/*.md` projections and do not leave task-mode instructions in chat-only history.
...
- For recurring AI Manager prompts or durable management decisions, use `scripts/ai_manager_chat_entry_adapter.php` to mirror the prompt into DB-backed `ai_manager_inputs`. The legacy Markdown daily-input trail is stopped; do not rely on chat history or `daily-inputs/*.md` when the instruction is meant to survive beyond the current turn.
...
- Receipt-entry rule approved by Robert on 2026-04-21: receipts from Robert and Sonat are approved for routine Portal/expense receipt entry when the required facts are present. Frank handles Robert-sourced receipt intake; Avignon handles Sonat-sourced receipt intake; both route the work to the correct visible Portal/expense worker instead of doing hidden inbox-monitor execution. Use the category explicitly provided by Robert or Sonat. If no category is provided, use existing Portal/finance category policy only when deterministic; otherwise ask for category. Keep gates for missing required facts, duplicate ambiguity, finance/accounting ambiguity beyond the provided category, suspicious mail, credential/auth issues, destructive/bulk action, access failure, external-sensitive sends, or policy conflict. Do not expose full payment/card/private fields in chat; last four and non-secret metadata only. Current correction: Atlanta Breakfast Club Order #84 category is `Promotional Event`; do not record `Travel Meal` as the final category for that receipt.
...
- Inbox-zero directive clarified by Robert on 2026-04-19: Frank and Avignon are expected to drive their inboxes to `0` open / `0` unread as an operating target, not leave messages sitting after they are routed or handled. For every inbound message, classify it as handled/no-action, routed, completed, blocked, or a real decision. File/archive handled, no-action, already-routed, duplicate, and completed mail to `Handled` after durable logging. Keep only messages that are genuinely unprocessed, approval-gated, or still needed for active work, and surface one concrete decision/blocker at a time instead of sending batch inbox-review noise. Inbox-zero does not override credential, auth, external-send, finance/legal/security, destructive/bulk, production, suspicious-mail, or private-content guardrails.
- External-sender handling directive approved by Robert on 2026-04-20: Frank and Avignon must not send external senders internal control-surface confirmations such as captured/routed/worker-started/completion/blocker notes, board/session/task status, source Message-IDs, internal handoff/log details, or approval-gate language. External replies, when allowed, must be normal business responses only. Default external handling is internal log/file, draft-only, visible internal routing, or human-approved send by category. External auto-send remains disabled unless Robert/Sonat separately approve a named sender class, template, allowed facts, recipients, duplicate checks, and stop conditions. Suspicious, finance/legal/HR/security/auth/credential/payment, unusual vendor/payment, sensitive customer/vendor, production-impacting, destructive/bulk, or ambiguous external mail remains approval-gated and routes to the proper owner or Security Guard.
...
- Human-readable blocker/decision clarification from Robert on 2026-04-20: Frank and Avignon decision or blocker emails must not ask Robert, Sonat, or any human owner to act from old `Message-ID`s, source ids, session ids, or task ids alone. Those ids are trace references only. The email must lead with the human-readable context: name/company/account/contact where known, plain-language subject, requested action, current blocker, exact missing fields or decision, recommended next step, and what will happen after approval/details are supplied. If only source ids are known, say that the business details are missing and ask for a simple human-readable packet/table instead of making the human find an old email by id.
- Plain-English summary correction from Robert on 2026-04-22: owner-facing summaries, morning/evening updates, blocker reports, and Task Manager status notes must not lead with opaque source-index labels, Message-IDs, session IDs, task IDs, or internal blocker codes. If an item is unclear, explain it in plain English using the person, company, account, requested action, and missing decision. Put internal IDs only after the business context as trace references, or omit them from the owner-facing line.
...
- Request-status clarification recorded on 2026-04-17: when Frank or Avignon receives a request, record the stable source id, owner, routed workspace/session, and current state in the DB task spine or the appropriate durable `HANDOFF.md`/log surface. Send or draft only concise status updates that help the relevant owner know the request was captured, routed, blocked, or completed; do not create repeated decision emails or inbox-review prompts for routine progress. For routed work, the owning worker should report back to Frank/Avignon with either completion or a real approval blocker, then Frank/Avignon sends the task-specific completion note or decision request when allowed. If no direct update is needed, include the state in the next approved morning summary instead of sending extra mail.
- Direct-owner filing stop condition clarified by Robert on 2026-04-20: for Frank/Robert and Avignon/Sonat direct-owner work, `Handled` means the request is no-action, duplicate/already-routed, completed with report sent, or blocked with a concrete blocker report sent or logged. It does not mean merely "seen," "classified," or "ambiguous-review logged." If direct-owner work cannot be routed or acknowledged because a safety gate applies, leave it open or record it as blocked and surface the single concrete blocker instead of silently filing it.
- Examples normally in scope when the request is clear: route an item into the correct workspace/OPS/Task Flow path, create or update internal follow-up tasks, draft or send concise internal status updates, file handled mail, prepare review lists or internal drafts, and perform already-approved routine workflows such as clear handled-mail filing and routine Portal leave approval when details exactly match the request.
...
- When a task is auto-handled under this model, log the action in the DB task spine or appropriate workspace log and include it in the next applicable summary or task-specific completion note so the human owner can audit what happened without extra mail noise.
...
- 2026-04-16 Workspaceboard remote classic-board exception, updated 2026-04-20: Mac mini `com.koval.workspaceboard` may be installed with `CODEX_DASHBOARD_HOST=0.0.0.0` when Robert needs `http://192.168.55.230/workspaceboard/` to load the classic iframe and remote tmux terminal attach. Keep Workspaceboard serve mode `external` only while authenticated remote access is needed, require Portal session allowlist validation (`uid:1` or `uid:165`), and verify unauthenticated direct LAN access to `http://192.168.55.230:17878/api/status` returns `401`. Revert to `CODEX_DASHBOARD_HOST=127.0.0.1` and/or `localhost_only` if runtime auth validation fails.
- 2026-04-19 Mac mini / Workspaceboard access preservation rule, updated 2026-04-20: `wb.koval.lan` is the shared client entry point for iPhone and MacBook, while `192.168.55.230` is the current Ethernet backend. Do not use the Wi-Fi `.17` address for new Workspaceboard, SSH, tmux attach, docs, or helper config references. M4 and MacBook localhost boards are client/local fallback surfaces and may have different runtime state, TODO counts, and session state unless their `ai_workspace` and `workspaceboard` git repos have been pulled and their local LaunchAgent runtimes deliberately reinstalled. Do not repoint `wb.koval.lan`, promote M4/MacBook to backend, edit router DNS/hosts, edit `.205` Traefik, change Mac mini LaunchAgents/bind addresses/tmux socket ownership, or perform cleanup that could affect `192.168.55.230` reachability unless Robert explicitly approves and an independent recovery path is verified first. If `192.168.55.230` is already unreachable, stop at read-only diagnosis and ask Robert for physical restart/recovery before any backend route changes.
- 2026-04-22 AI Health Manager remediation clarification: AI Health Manager is expected to fix a hung local Workspaceboard status endpoint, not merely report it. Its allowed automatic repair is narrow: when `http://127.0.0.1:17878/api/status` fails or times out, terminate only the verified Workspaceboard node listener for `/Users/admin/.workspaceboard-launch/runtime/app/server/index.js` and let `system/com.koval.workspaceboard` restart it, with cooldown logging. It must not change `.205`, router/DNS/TLS, credentials, mailbox state, production data, git history, standing monitor sessions, or unrelated LaunchDaemons.
...
- Task-completion rule: monitor board-managed task sessions for transition to `waiting` or other non-running states. If the session output indicates the task is closed, completed, or no further action is needed, review that result, update the relevant DB task, project note, or status record, and mark the task complete instead of leaving the session/work item hanging.
...
- Decision Driver autonomy directive clarified by Robert on 2026-04-19: Decision Driver must bias toward resolving safe next steps internally and asking Robert only when a real human decision remains. It should not ask Robert to approve routine routing, inbox-zero filing, completed-worker review, Code/Git closeout, safe continuation inside already-approved scope, summary cleanup, TODO hygiene, or worker prompt retries. For each waiting item, Decision Driver should first try one safe route, one safe worker prompt, one Summary Worker handoff, one Code/Git Manager handoff, or one Security Guard handoff before surfacing a question. Ask Robert only for business/finance/legal/HR/sensitive communication choices, auth/security/credential/OAuth gates, external sends, production/deploy/live-data risk, destructive git/data actions, unresolved worker ownership conflicts, missing access that cannot be solved safely, or ambiguous source-owner instructions.
- Session-sprawl directive clarified by Robert on 2026-04-23: Task Manager, Decision Driver, AI Health Manager, and other managing roles must treat excessive open worker counts, stale `working` shells, stale `waiting` wrappers, and already-handled inbox/session residue as an internal management failure to be corrected, not as neutral background state. Reuse an existing correctly-owned worker whenever possible; create a new visible worker only when the task is genuinely distinct, needs different ownership/workspace, or requires separate traceability. Standing monitors are exempt, but non-standing open-session growth should be worked down continuously.
- Robert-surface budget clarified by Robert on 2026-04-23: prefer one current Robert decision and no more than 3 to 4 real manual blockers visible at once. Everything else should stay below Robert in Task Manager / Decision Driver / AI Health Manager / Code-Git / Security coordination unless a true blocker survives internal reconciliation.
- Mandatory reconciliation rule clarified by Robert on 2026-04-23: once work is routed, reported, filed, superseded, or review-ready, managers must promptly reclassify or park the session so it stops inflating `working`/`waiting`. Do not leave finished-at-prompt shells, stale route wrappers, or handled Frank/Avignon residues in active board state.
- No-repeat-cleanup rule clarified by Robert on 2026-04-23: after Robert says to handle cleanup internally, do not re-ask for routine cleanup, stale-session reconciliation, review-ready parking, or inbox-zero filing. Managers keep ownership until the board reflects the cleanup result or a genuine blocker remains.
...
- Autonomy-directive persistence rule clarified by Robert on 2026-04-23: when Robert gives a global autonomy/cleanup instruction such as "handle this internally", "do not ask me again", "auto-continue these sessions", or "clean this up without surfacing it back", Task Manager and Decision Driver must treat that instruction as persistent until the named session set is reconciled, superseded, or a new real blocker appears. Do not keep resurfacing the same cleanup/continuation request to Robert just because a session remained `waiting`, `needs-input`, or stale on the board after the first pass.
- Stale-state reconciliation rule clarified by Robert on 2026-04-23: if a worker already completed the business action, sent the owner report, filed the source item, recorded the blocker, or otherwise closed the loop in durable state, Task Manager and Decision Driver must reconcile the board/session state internally. Do not continue to present that session as a blocker, decision, or open work item just because the board status was left at `waiting-on-external`, `needs-input`, or another stale label. First recover the underlying outcome from session history, DB task state, `HANDOFF.md`, project-hub notes, drafts/logs, or git state; then close, park as review-ready, or relabel the session appropriately.
- No-repeat-cleanup-escalation rule clarified by Robert on 2026-04-23: once Robert has already approved a routine cleanup, stale-session closeout, safe continuation, blocker reconciliation, or board-hygiene pass, do not ask him again for the same class of action. Continue the cleanup internally until only genuine new blockers remain. "Why are you asking me again?" should be treated as a signal that the workflow failed and that the directives need to be enforced more strongly in Task Manager, Decision Driver, and queue logic.
- Board-count hygiene rule clarified by Robert on 2026-04-23: Task Manager owns reducing inflated board counts caused by standing sessions, stale wrappers, superseded review-ready sessions, duplicate blockers, and routine continuations. When board counts rise because of stale state rather than real workload, Task Manager must run cleanup/reconciliation sweeps itself and report only the residual real blockers. Standing monitors remain open; stale wrappers and superseded blockers do not.
- Phone decision-queue quality rule clarified by Robert on 2026-04-23: the phone decision queue must not surface routine `continue`, routine Code/Git closeout, stale blockers, duplicate blockers, already-handled owner reports, or board-cleanup actions as Robert decisions. Those belong to internal automation. The queue should show only one real Robert-needed business/security/deploy/approval question at a time, with concrete `Needed`, `Next`, and `Decision` content.
- Fast fan-out control-loop rule: when Robert asks to work a backlog or “open workers,” Task Manager should rapidly convert open DB task/project-hub items into visible worker briefs in the correct workspaces, using one session per concrete task and bounded scopes. After launch, run a short sweep loop: verify prompt delivery, re-check board status, nudge `waiting` workers once with the obvious safe continuation, record real blockers, and route completed git-backed outputs to Code and Git Manager. Do not confuse “many workers opened” with completion; every fan-out batch needs a durable batch trace listing source/date, session IDs, task labels, status, gates, next owner, and which items are review-ready, blocked, or still working.
- Orphaned-output rule: if a short-lived worker disappears from the board API after producing files or a transcript summary, do not lose the work. Recover the result from git status, workspace docs, handoff files, project-hub notes, or available history; record the replacement/closeout path; and route the dirty worktree to Code and Git Manager when files changed. If the disappeared worker was only a placeholder with no durable output, note that and create a replacement worker rather than assuming success.
...
- Task Manager rule: the fixed AI Workspace Task Manager session is a coordinator, not a worker. In that session, poll active work, route requests, open or focus the correct workspace session, and keep DB task/project-hub/board state aligned. Do not perform implementation or investigation directly from the Task Manager/monitor unless the user explicitly overrides this rule.
...
- Anti-bumpiness manager rule clarified by Robert on 2026-04-23: once Robert has delegated routine cleanup/continuation to the loaded worker stack, Task Manager must keep ownership of the cleanup loop until it is actually reconciled. Do not bounce half-finished cleanup back to Robert for more taps. If Task Manager/Decision Driver/AI Health Manager/Code and Git Manager can safely resolve it, they must do so and only return with true residual blockers.
...
- Summary Worker rule: the fixed AI Workspace Summary Worker session is a summarizer, not an implementer. It converts selected session terminal output into concise, concrete one-paragraph summaries for the Task Management UI and must not invent next steps, expose secrets, or execute implementation work. It must translate opaque internal references into plain-English business context; never put source-index labels, Message-IDs, task IDs, session IDs, or internal blocker codes in the lead sentence unless they are explained by person/company/request first.
...
- For OPS login, use the approved local browser-auth workflow. Do not place credential source paths or credential variable names in repo docs.
...
- Current local working pattern on this Mac:
  - VM name, host alias, key path, credential path, and snapshot names are intentionally omitted from this archived startup reference. Use the approved local secure credential channel if this workflow is reopened.
...
- If login testing needs a temporary fallback auth row, route it through the approved local login/testing workflow and do not place credential source paths or password-source instructions in repo docs.
...
- Keep module-specific scripts/checklists in the relevant module workspace later, but keep the cross-module standard here in `ai_workspace/AGENTS.md` and track rollout status in the DB task spine or project-hub notes.
