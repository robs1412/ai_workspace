...
- `2026-06-07-shared-vanessa-style-fast-path-reliability.md`: shared mechanic for making Frank, Avignon, Ezra, Naomi, Asher, Venetia, and future approved email workers behave more like Vanessa operationally: short route tables, explicit `routine-if-clear` classes where already approved, one of five finish states, and proof-surface defaults. This is docs-only and does not grant new send, filing, body-read, runtime, credential, finance-system, or production authority.
  - 2026-06-25 lane anchor: COTeam staffing/shift/schedule/team-member/tasting/Outreach follow-through routes to Vanessa / National Outreach by default; finance/BID/QBO/Square/payroll/invoice/reimbursement/receipt/financial-reporting follow-through routes to Naomi / finance by default. Mixed packets should split routes or block with one exact ownership question.
...
- 2026-06-12 accessible-packet correction: owner-facing review packets, research summaries, tables, and report files are not complete if they remain only as local `.md`, `.csv`, or `/Users/werkstatt/...` paths. Publish the readable packet to Google Drive AI Cloud, Papers/Papers Pages, or a repo-backed access-gated HTML page, verify readback, then send the full accessible link. Local Markdown/CSV/HTML may be working copies only.
